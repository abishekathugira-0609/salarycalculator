...
// Output
const outputDir = path.join(process.cwd(), "data", "pages", "2026");
if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true });

// ==============================
// FEDERAL TAX (2026 – PROJECTED)
// ==============================
function calculateFederalTax(income) {
  const standardDeduction = 15350; // projected (+3%)
...
for (const salary of salaries) {
  for (const state of states) {
    ...
    fs.writeFileSync(
      path.join(outputDir, `${salary}_${state.code}_single_2026.json`),
      JSON.stringify(json, null, 2)
    );
  }
}

console.log("✅ 2026 salary pages generated (projected estimates)");
